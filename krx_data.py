...
def jonmok_history(kos_cd = 'kosdaq', corp_cd='', strtDd=''):
    ...
    if len(temp) < 5:
        ...
        d_1 = temp[-1]['TDD_CLSPRC'].replace(',','') #상장첫날
        d_2 =temp[-2]['TDD_CLSPRC'].replace(',','')
        d_3 =temp[-3]['TDD_CLSPRC'].replace(',','')
        d_4 =temp[-4]['TDD_CLSPRC'].replace(',','')
        d_5 = temp[-5]['TDD_CLSPRC'].replace(',','')
        try:
            d_30 = temp[-20]['TDD_CLSPRC'].replace(',','') #20거래일 뒤. 1달 뒤
            d_len = True
        except: #상장 20거래일이 안됐으면 걍 d_30, d_60은 최신값. d_len 은 False
            d_30 =temp[0]['TDD_CLSPRC'].replace(',','')
            d_len = False
        d_60 =temp[0]['TDD_CLSPRC'].replace(',','')
        return [d_1, d_2, d_3, d_4, d_5, d_30, d_60, d_len]
